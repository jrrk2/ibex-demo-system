...
local FILES = {
  ...
  "/home/jonathan/v7-johnson-demo/ibexsoc/vendor/pulp_riscv_dbg/src/dmi_jtag_tap.sv",
...
if sram and strlen(sram) > 0 then
  p = svd.parse_spec("verible", "ibex_mini_system", FILES, "SRAMInitFile=\""..sram.."\"")
else
  p = svd.parse("verible", "ibex_mini_system", FILES)
end
...
-- Skip memlower for BEHAV_VERILOG and for whole-SoC CYCLESIM (the RAM must stay
-- a behavioural array — a RAMB black box can't be simulated by Cyclesim).
if not (os.getenv("BEHAV_VERILOG") or os.getenv("CYCLESIM_NOMEMLOWER")) then p=svd.memlower(p) end
p=svd.srl_infer(p)
-- OPT_LOGIC=1: architecture-independent logic optimisation (const-prop + DCE +
-- CSE, gate-map-safe: no SSA / no reg-inference) on the final BIR before
-- gate_map.  Complements the arch-DEPENDENT knobs (GATE_MAP_MODE/AIG_BALANCE/
-- LUTPACK/MFS2) that act during LUT covering.
if os.getenv("OPT_LOGIC") then p=svd.optimize_logic(p); print("OPT_LOGIC applied") end
...
if os.getenv("BEHAV_VERILOG") then
  svd.write_verilog(p, (os.getenv("W") or "/tmp/svs_ibex_mini") .. "/ibex_mini_sim_dut.v")
  print("WROTE behav verilog")
  return
end
-- CYCLESIM_SPEC=<module;drives>: in-process Hardcaml Cyclesim of ONE module's
-- create_circuit output (portable; no xsim/iverilog) — bisect create_circuit
-- lowering bugs directly.  e.g.
--   CYCLESIM_SPEC='ibex_fetch_fifo;cycles=20;rst_ni=pulselow:2;clear_i=pulse:5;\
--                  in_valid_i=1;out_ready_i=1;in_rdata_i=count:256'
if os.getenv("CYCLESIM_SPEC") then
  print(svd.cyclesim(p, os.getenv("CYCLESIM_SPEC")))
  return
end
-- EMIT_CREATE_CIRCUIT=<dir>: emit every module's create_circuit output as
-- Verilog WITHOUT gate_map (debug right after Hardcaml create_circuit; no
-- wasted AIG/LUT).  Verilate the result to simulate the Hardcaml output.
...
-- LUT cover width: 6 (LUT6 only, default) or 7/8 to also pack wide functions
-- into LUT6+MUXF7 / 4xLUT6+2xMUXF7+MUXF8 (shallower wide cones).  GATE_MAP_K=8.
local kcov = tonumber(os.getenv("GATE_MAP_K") or "6") or 6
while k<=cnt do
  local M=NT[k]
  print("gate_map "..M.." (k="..kcov..")")
  result = svd.splice(result, M, svd.mapped_to_prog(svd.gate_map(svd.pick(result,M),kcov,0)))
  k=k+1
end
...
if os.getenv("HIER_EDIF") then
  svd.write_hier_edif(result, "ibex_mini_system", out)
else
  local net = svd.flatten_struct(result, "ibex_mini_system")
