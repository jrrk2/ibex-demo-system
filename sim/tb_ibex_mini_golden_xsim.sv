`timescale 1ns/1ps
module tb_golden;
  logic clk=0, rst_n=0, tck=0, trst_n=0, tms=1'b1;
  logic [7:0] gp_o, prev=8'hxx;
  integer ticks=0;
  always #10 clk = ~clk;
  always #7  tck = ~tck;
  ibex_mini_system #(.SRAMInitFile("/home/jonathan/v7-johnson-demo/ibexsoc/sw/mini/led_count_sim.vmem")) dut (
    .clk_sys_i(clk), .rst_sys_ni(rst_n), .gp_i(8'h00), .gp_o(gp_o),
    .tck_i(tck), .tms_i(tms), .trst_ni(trst_n), .td_i(1'b0), .td_o());
  initial begin
    rst_n=0; trst_n=0; tms=1'b1; #203; trst_n=1'b1; #100; tms=1'b0; rst_n=1'b1;
    $display("[tb] reset released @ %0t", $time);
  end
  always @(gp_o) if (rst_n===1'b1 && gp_o!==prev) begin
    $display("[tb] t=%0t LED=0x%02x", $time, gp_o); prev=gp_o; ticks=ticks+1;
    if (ticks>=8) begin $display("[tb] PASS %0d", ticks); $finish; end
  end
  initial begin #400000; $display("[tb] TIMEOUT ticks=%0d LED=0x%02x", ticks, gp_o); $finish; end
endmodule
